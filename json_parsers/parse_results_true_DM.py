...
def main(results_dir):
    ...
    for f in files:
        ...
        row = {
            ...
            "Full acc": R["full"]["test_acc"],
            "Rand acc": R["random"]["test_acc"],
            "Herd acc": R["herd"]["test_acc"],
            "DM acc": R["dm"]["test_acc"],
            "embed_hidden": hdim,
            "embed_dim": edim,
            "iters": iters,
            "lr": lr,
    ...
    out_md = os.path.join(results_dir, "summary_acc_only.md")
    with open(out_md, "w") as md:

        md.write("| Dataset | IPC | Classes | Norm | Full acc | Rand acc | Herd acc | DM acc | embed_hidden | embed_dim | iters | lr | file |\n")
        md.write("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n")

        for r in rows:
            md.write(
                f"| {r['Dataset']} | {r['IPC']} | {r['Classes']} | {r['Norm']} | "
                f"{r['Full acc']:.4f} | {r['Rand acc']:.4f} | {r['Herd acc']:.4f} | {r['DM acc']:.4f} | "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", type=str, default="./results_trueDM_LN_IPC10", help="Directory with JSON runs.")
